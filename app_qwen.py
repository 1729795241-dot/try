...
# 标题
st.markdown('<div class="main-header">🤖 财务报表智能分析系统 (通义千问版)</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">基于阿里云通义千问Qwen-Long - 1000万Token超长上下文</div>', unsafe_allow_html=True)
...
with tab1:
    ...
    uploaded_file = st.file_uploader(
        "选择PDF文件",
        type=['pdf'],
        help="支持上传任何类型的PDF（文本型或扫描版），文件大小不超过150MB"
    )
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 开始分析", type="primary", use_container_width=True):
            if not api_key:
                st.error("❌ 请先在侧边栏设置通义千问API密钥")
            else:
                # 创建工作流
                with st.spinner("🔄 通义千问正在分析PDF，请稍候..."):
                    try:
                        workflow = QwenFinancialWorkflow(api_key=api_key, output_dir=output_dir)
...
<div style='text-align: center; color: #666; padding: 1rem;'>
    <p>财务报表智能分析系统 v3.0.0 (通义千问版) | Powered by 阿里云通义千问 Qwen-Long</p>
    <p>1000万Token超长上下文 | 国内直接访问 | 免费额度100万Token</p>
</div>
""", unsafe_allow_html=True)
